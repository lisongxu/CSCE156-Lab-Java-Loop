...
\mintinline{java}{List} and \mintinline{java}{Set} collections.
...
\begin{enumerate}
  ...
  \item The \mintinline{java}{Child} class has already been implemented
	...
	\mintinline{java}{List}.  
	
	Similar to an array, a \mintinline{java}{List} stores elements using
	0-indexing.  Each one can be accessed using the \mintinline{java}{get}
	method.  For example, \mintinline{java}{kids.get(i)} gets the $i$-th
	kid.  You could also use an \emph{enhanced for loop} to iterate over
	the elements in a \mintinline{java}{List}.  
	
  \item Write code to iterate over the array, compute the child tax 
	credits and output a table similar to the one above.  
	
	Each child is an \emph{instance} of the \mintinline{java}{Child} class.
	...
	``getter'' method.  For example, to get the age of \mintinline{java}{tom}
	you can use the syntax \mintinline{java}{tom.getAge()}
